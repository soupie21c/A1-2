...
argparse
3-1. 환경변수 사용 이유
.env 파일과 환경변수를 사용하면 API 키를 코드에 직접 쓰지 않아 보안에 안전하고,
개발/배포 환경에 따라 값을 쉽게 바꿀 수 있어 운영이 편리합니다.
3-2. GET / POST 이유
GET은 Kakao Local API처럼 데이터를 조회할 때 사용하고,
POST는 LLM 호출처럼 요청 본문을 보내는 작업에서 주로 사용합니다.
3-3. JSON 강제 이유
LLM 응답을 JSON으로 강제하면 구조화된 후처리, 키 검증, 파일 저장이 쉬워집니다.
또한 프로그램에서 자동 파싱과 오류 처리를 안정적으로 할 수 있습니다.
3-4. 401/403 디버깅
401/403 오류가 발생하면 다음을 확인합니다.
- API 키가 올바른지
- 권한이 활성화되어 있는지
- 쿼터가 초과되지 않았는지
- 헤더 형식이 맞는지
